...
const update = async (user) => {
  try {
    const { data } = await axios.post(`/api/updateUser/${user.userId}` , user);
    if (data && data.userId) {
      console.log(
        "authService:update() Success: ",
        user.username,
        " successfully updated."
      );
      return { isUpdated: true, error: null };
    } else {
      console.error("authService:update() Error: ", data);
      return { isUpdated: false, error: data };
    }
  } catch (error) {
    console.error("authService:update() Error: ", error.response.statusText);
    return { isUpdated: false, error: error.response.statusText };
  }
};
